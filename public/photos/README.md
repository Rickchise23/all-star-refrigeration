...
- **Gallery** uses everything in `OWNER_PHOTO_FILES` except the hero van, strip images, service-detail-only shots (`REPAIR_PAGE_GALLERY_EXCLUDE`, `INSTALL_PAGE_GALLERY_EXCLUDE`, `MAINTENANCE_PAGE_GALLERY_EXCLUDE`; `HEATING_PAGE_GALLERY_EXCLUDE` is empty), and **`HOME_GALLERY_EXCLUDE`** (**`image11`**, **`image15`**).
